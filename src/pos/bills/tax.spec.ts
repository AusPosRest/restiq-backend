...
describe('computeTax', () => {
  it('IN + CGST/SGST profile: splits 5% into 2.5% CGST + 2.5% SGST, summing to taxMinor', () => {
    const result = computeTax({ country: 'IN', taxProfile: 'India GST - CGST/SGST split', compositionScheme: false, subtotalMinor: 20000n })
  ...
  it('IN with no recognizable profile (e.g. a tenant with no tax registration row) defaults to the CGST/SGST split', () => {
    const result = computeTax({ country: 'IN', taxProfile: '', compositionScheme: false, subtotalMinor: 20000n })
    expect(result.taxMinor).toBe(1000n)
    expect(result.breakdown.map((l) => l.label)).toEqual(['CGST', 'SGST'])
  })

  it('IN + IGST profile: a single 5% IGST line', () => {
    const result = computeTax({ country: 'IN', taxProfile: 'Interstate - IGST', compositionScheme: false, subtotalMinor: 20000n })
    expect(result.pricesIncludeTax).toBe(false)
    expect(result.taxMinor).toBe(1000n)
  ...
  it('IGST match is case-insensitive', () => {
    const result = computeTax({ country: 'IN', taxProfile: 'igst', compositionScheme: false, subtotalMinor: 10000n })
  ...
  it('IN + compositionScheme: zero tax, empty breakdown, the statutory note - even for an otherwise-IGST profile', () => {
    const result = computeTax({ country: 'IN', taxProfile: 'IGST', compositionScheme: true, subtotalMinor: 20000n })
  ...
  it('AU: single inclusive 10% GST line, tax backed out of the subtotal', () => {
    const result = computeTax({ country: 'AU', taxProfile: 'Australia GST', compositionScheme: false, subtotalMinor: 11000n })
  ...
  it('AU: compositionScheme is an IN-only concept and has no effect', () => {
    const withComposition = computeTax({ country: 'AU', taxProfile: 'Australia GST', compositionScheme: true, subtotalMinor: 11000n })
  ...
  it('rounding: IN CGST/SGST on a subtotal where the two lines round to the same figure', () => {
    ...
    const result = computeTax({ country: 'IN', taxProfile: '', compositionScheme: false, subtotalMinor: 30n })
  ...
  it('rounding: IN CGST/SGST where CGST alone would round up to more than half of taxMinor, SGST still comes out non-negative', () => {
    ...
    const result = computeTax({ country: 'IN', taxProfile: '', compositionScheme: false, subtotalMinor: 10n })
  ...
  it('rounding: IGST rounds half up at the 5% boundary', () => {
    ...
    const result = computeTax({ country: 'IN', taxProfile: 'IGST', compositionScheme: false, subtotalMinor: 10n })
    expect(result.taxMinor).toBe(1n)
  })

  it('rounding: AU GST on a subtotal not evenly divisible by 11', () => {
    // 100 * 10/11 = 90.909... -> round-half-up 91; tax = 100 - 91 = 9.
    const result = computeTax({ country: 'AU', taxProfile: 'GST', compositionScheme: false, subtotalMinor: 100n })
    expect(result.taxMinor).toBe(9n)
  })

  it('zero subtotal produces zero tax in every branch', () => {
    expect(computeTax({ country: 'IN', taxProfile: '', compositionScheme: false, subtotalMinor: 0n }).taxMinor).toBe(0n)
    expect(computeTax({ country: 'IN', taxProfile: 'IGST', compositionScheme: false, subtotalMinor: 0n }).taxMinor).toBe(0n)
    expect(computeTax({ country: 'IN', taxProfile: '', compositionScheme: true, subtotalMinor: 0n }).taxMinor).toBe(0n)
    expect(computeTax({ country: 'AU', taxProfile: '', compositionScheme: false, subtotalMinor: 0n }).taxMinor).toBe(0n)
  })
})
